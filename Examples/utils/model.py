...
class Model:
    ...
    def fit_fedadmm(self, X, y, z_beta, z_gamma, u_beta, u_gamma):
        # Unpack structured array 
        event, duration = zip(*y)
        ...
        # TODO: check that has correct dims!!!!
        ...
        def _augmented_lagrangian_loss():
            ...
            # ADMM Augmented Lagrangian penalty
            penalty_beta = tf.norm(beta - z_beta + u_beta, ord=2)
            penalty_gamma = tf.norm(gamma - z_gamma + u_gamma, ord=2)
            
            return nll + reg_beta + reg_gamma + 0.5 * (penalty_beta + penalty_gamma)
